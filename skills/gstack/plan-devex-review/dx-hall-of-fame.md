...
Reference implementations: hierarchical storage (global vs per-project vs per-session), progressive consent flows with marker files, append-only JSONL audit trails, auto-upgrade with cache + snooze backoff.
